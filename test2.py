...
class CropRowEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(
            low=np.array([0, -1.5]),
            high=np.array([self.num_corridors - 1, self.corridor_length - 0.5]),
            dtype=np.int32
        )
        ...
        if action_type == 0:
            ...
            if at_end:
                if 0 <= value < self.num_corridors and value != corridor:
                    corridor = value
                    print("pos", pos)
                    self.orientation = None  # reset orientation after switching
