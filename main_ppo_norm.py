...
class CropRowEnv(gym.Env):
    ...
    The observation is now normalized in the vertical (corridor length) dimension:
      - The robot's vertical position (state[1]) is scaled from its native range of [-1, corridor_length-1]
        to [0,1] using: normalized_y = (pos + 1) / self.corridor_length.
      - The goal’s vertical coordinate (sampling_y) is scaled from its native range of [0, corridor_length-2]
        to [0,1] using: normalized_goal_y = sampling_y / (self.corridor_length - 2).
    (All internal computations and the rendering remain unchanged.)
    """
    metadata = {"render.modes": ["human"]}

    def __init__(self,
                 num_crop_rows=5,      # Number of crop rows (vertical lines). Corridors = num_crop_rows - 1.
                 corridor_length=12,   # Original corridor length (vertical positions: -1 to corridor_length-1).
        ...
        # The internal corridor length is increased by 2 (as before) for handling boundaries.
        self.corridor_length = corridor_length + 2  

        # --- Observation Space ---
        # The observation is a 6-element vector:
        # [crop_row (corridor center), normalized vertical position, orientation, driving_direction (crop row),
        #  goal crop row, normalized goal vertical position]
        #
        # For normalization:
        #   - The robot’s vertical position pos is in [-1, self.corridor_length-1]. We map -1 -> 0 and
        #     (self.corridor_length-1) -> 1 via (pos + 1) / self.corridor_length.
        #   - The goal’s vertical coordinate sampling_point[1] is in [0, self.corridor_length-2]. We map 0 -> 0 and
        #     (self.corridor_length-2) -> 1 via sampling_point[1] / (self.corridor_length - 2).
        self.observation_space = spaces.Box(
            low=np.array([0.5, 0.0, -1.0, 0.5, 0, 0], dtype=np.float32),
            high=np.array([self.num_corridors - 0.5, 1.0, 1.0, self.num_corridors - 0.5, 1.0, 1.0], dtype=np.float32),
            dtype=np.float32
        )
        ...
        # Variables set by the agent:
        self.orientation = None      # robot orientation (0: upward, 1: downward)
        self.drivingd = None         # vertical move command: 0 for forward, 1 for backward

        # Other environment variables:
        self.state = None            # (crop_row, pos) with pos in original (unnormalized) coordinates
        self.sampling_point = None   # (crop row, vertical position) in original coordinates
        self.path = []  # store robot positions (crop_row, pos) in original coordinates
    ...
    def reset(self, seed=None, options=None):
        ...
        # Randomize the start position (using original coordinate system)
        self.state = (np.random.randint(self.num_corridors - 1) + 0.5,
                      np.random.randint(0, self.corridor_length - 2))
        self.state = (0.5,5)
        # Randomize the goal position (also in original coordinates)
        ...
        obs = np.array([
            (self.state[0]) / (2 * self.num_corridors),
            (self.state[1] + 1) / self.corridor_length,  # normalized vertical position
            -1.0,      # orientation undefined
            (self.state[0]) / (2 * self.num_corridors),  # driving direction (here still same as crop row)
            self.sampling_point[0] / self.num_corridors,
            (self.sampling_point[1]+1) * (1/self.corridor_length)  # normalized goal vertical position
        ], dtype=np.float32)
        return obs, {}

    def _get_robot_coords(self):
        """Return the robot's current (x, y) coordinates for visualization (original coordinates)."""
    ...
    def step(self, action):
        ...
        if not at_end and self.orientation is not None:
            if ori_action != self.orientation:
                reward -= 0.5  # Penalize changing orientation within a row.
            else:
                reward += 0.05
        ...
        if not at_end and self.previous_action is not None:
            ...
            if (move_action == 0 and prev_move == 1) or (move_action == 1 and prev_move == 0):
                reward -= 0.5
            elif (move_action == 0 and prev_move == 0) or (move_action == 1 and prev_move == 1):
                reward += 0.05
        ...
        if move_action < 2:
            ...
            # Here, we compute a target crop row (using the original coordinate system)
            value = move_action - 1.5  # target corridor index (e.g., 0.5, 1.5, etc.)
        else:
            reward = -0.6
        ...
        # Corridor switching: if move_action >= 2.
            ...
            if at_end:
                if 0.5 <= value < self.num_corridors:
                    corridor = value
                    self.turn = True
                    # When switching corridors, the agent will select a new orientation in the next step.
                    self.drivingd = None
                    reward = -0.1
                else:
                    reward = -0.5
        ...
        # Update the internal state (using original coordinates).
        ...
        obs = np.array([
            corridor / (2 * self.num_corridors),
            (pos + 1) / self.corridor_length,  # normalized vertical position
    ...
    def render(self, mode="human"):
        """Render the environment using the original (unnormalized) coordinates."""
        self.ax.clear()
...
if __name__ == "__main__":
    num_crop_rows = 5
    corridor_length = 5  # This is the "base" corridor length; internally it becomes corridor_length+2.
    env = CropRowEnv(num_crop_rows=num_crop_rows, corridor_length=corridor_length)
    state, _ = env.reset()
    print("Initial state (normalized obs):", state, "Orientation:", env.orientation, "Goal:", env.sampling_point)
    print("Accumulated Reward: {:.2f}".format(env.total_reward))
    
    done = False
    while not done:
        # For the first step, if drivingd is not set, choose a random vertical action.
        ...
        # For the orientation element, choose randomly (for testing).
        ori_action = np.random.choice([0, 1])
        action = np.array([ori_action, move_action])
        
        state, reward, done, _, _ = env.step(action)
        print(f"Action: {action} -> State (normalized obs): {state}, Orientation: {env.orientation}, Step Reward: {reward:.2f}, Accumulated Reward: {env.total_reward:.2f}, Done: {done}")
        env.render()
